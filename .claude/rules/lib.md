...
**Shared helpers**:
- `buildContextHelpers(geo)` — AI context with user theme, geo-IP location (city, lat/lng, H3 cells at res 1/3/5/7), behavior rules, DuckDB v1.5 notes (geometry auto-detection, synthetic lat/lng warning, lambda syntax, spatial filter pushdown, TRY_CAST broken, no same-name aliasing), dataset paths, component tips, anti-injection rules. Coordinate order: `lat` = latitude (N/S), `lng` = longitude (E/W). H3: `h3_latlng_to_cell(lat, lng, res)`. Spatial: `ST_Point(lng, lat)`.
