 ...
 */
function isValidPromptData(promptData: unknown): promptData is { messages: PromptMessage[] } {
    ...
    return (
      <TooltipProvider>
        <Tooltip
          ...
          side="top"
          className={cn("bg-muted text-foreground", promptError && "bg-destructive text-destructive-foreground")}
        >
          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              <button
                ref={ref}
                type="button"
                className={cn(buttonClasses, promptError && "border-destructive text-destructive")}
                aria-label="Insert MCP Prompt"
                data-slot="mcp-prompt-button"
                {...props}
              >
                {promptError ? <AlertCircle className="w-4 h-4" /> : <FileText className="w-4 h-4" />}
              </button>
  ...
  return (
    <>
      {promptList.map((promptEntry) => (
        ...
        >
          <span className="font-medium truncate max-w-full">{promptEntry.prompt.name}</span>
          {promptEntry.prompt.description && (
            <span className="text-xs text-muted-foreground truncate max-w-full">{promptEntry.prompt.description}</span>
          )}
        </DropdownMenu.Item>
      ))}
    </>
...
function ResourceListContent({
  ...
  if (isLoading) {
    return <div className="px-2 py-8 text-center text-sm text-muted-foreground">Loading resources...</div>;
  }
    ...
    <>
      {filteredResources.map((resourceEntry) => (
        <DropdownMenu.Item
          ...
          onSelect={() => {
            onSelectResource(resourceEntry.resource.uri, resourceEntry.resource.name || resourceEntry.resource.uri);
          }}
        >
          <div className="flex items-start justify-between w-full gap-2">
            <div className="flex-1 min-w-0">
              <div className="font-medium truncate">{resourceEntry.resource.name ?? "Unnamed Resource"}</div>
              <div className="text-xs text-muted-foreground truncate font-mono">{resourceEntry.resource.uri}</div>
  ...
  ({ className, onInsertResource, value: _value, ...props }, ref) => {
    ...
    const filteredResources = React.useMemo(() => {
      ...
      return resourceList.filter((entry) => {
        ...
        return [uri.includes(query), name.includes(query), description.includes(query)].some(Boolean);
      });
    ...
    return (
      <TooltipProvider>
        <Tooltip content="Insert MCP Resource" side="top" className="bg-muted text-foreground">
