...
export const h3MapSchema = z.object({
  ...
  hexColumn: z
    .string()
    .optional()
    .describe("Column name for H3 hex string in query result (default: 'hex')"),
  valueColumn: z
    .string()
    .optional()
    .describe("Column name for the numeric value to color by (default: 'value')"),
  ...
  colorMetric: z
    .string()
    .optional()
    .describe("Legend label for the color metric (e.g. 'Population Density')"),
  colorScheme: z
  ...
  extruded: z
    .boolean()
    .optional()
    .describe("3D extrusion based on value"),
});
...
export const H3Map = React.forwardRef<HTMLDivElement, H3MapProps>(
  (props, ref) => {
    const {
      title,
      queryId,
    ...
    const handleBoundsChange = React.useCallback(
      async (bbox: [number, number, number, number]) => {
        ...
        } catch { /* h3-js not available — skip spatial filter */ }
      },
        ...
        <div className="px-4 py-2 border-t bg-muted/10 flex items-center gap-3 flex-shrink-0">
          {values.length > 0 && (
            <>
              ...
              <div
                className="flex-1 h-3 rounded-full max-w-sm"
          ...
          {hasData && (
            <span className="text-sm text-muted-foreground ml-auto">
              {hexagons.length.toLocaleString()} hexagons
            </span>
          )}
        </div>
      </div>
